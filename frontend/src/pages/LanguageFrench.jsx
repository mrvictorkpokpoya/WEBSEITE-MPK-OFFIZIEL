...
import { FRENCH_COURSES } from "@/lib/data";
...
export default function LanguageFrench() {
  const { t } = useTranslation();
  return (
    <>
      <PageHero
        eyebrow={t("languages.french_section_eyebrow")}
      ...
      <section className="max-w-[1400px] mx-auto px-4 sm:px-5 lg:px-10 py-12">
        <div className="flex items-center gap-3 mb-3">
          ...
          <Eyebrow>{t("languages.sublevels_title")} · A1.1 → C2.2</Eyebrow>
        </div>
        <h2 className="font-serif text-2xl sm:text-3xl text-[#2F0808] uppercase tracking-tight">{t("languages.french_title")}</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 mt-8">
          {FRENCH_COURSES.map((c) => <PlaceholderCard key={c.level} c={c} />)}
        </div>
      </section>
      ...
      <section className="max-w-[1400px] mx-auto px-4 sm:px-5 lg:px-10 py-16 text-center">
        ...
        <div className="mt-6"><CTA to="/contact" label={t("training_plus.coming_notify")} testid="french-notify-cta"/></div>
      </section>
    </>
  );
}
