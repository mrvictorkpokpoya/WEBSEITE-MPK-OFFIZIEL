    ...
    <article id={c.id} data-testid={`campus-card-${c.id}`} className="mpk-card flex flex-col bg-white">
      <div className="bg-[#580505] text-white px-6 py-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[11px] tracking-[0.2em] uppercase text-[#C4D2ED]">{c.city}{c.flagship ? t("campuses.flagship_suffix") : ""}</div>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 space-y-3 flex-1">
        ...
        <a href={`https://wa.me/${c.whatsapp}`} target="_blank" rel="noreferrer" data-testid={`campus-whatsapp-${c.id}`} className="inline-flex items-center gap-2 text-sm font-medium text-[#580505] border border-[#580505]/30 px-3 py-2 hover:bg-[#580505] hover:text-white transition mt-1">
          <MessageCircle size={14} /> {t("campuses.whatsapp_community")}
        </a>
      </div>

      <div className="px-5 sm:px-6 pb-6">
        <div className="text-[10px] tracking-[0.2em] uppercase text-[#550000] font-medium mb-2 flex items-center gap-2"><MapIcon size={12} /> {t("campuses.map_label")}</div>
        {c.map ? (
          <a href={c.map} target="_blank" rel="noreferrer" data-testid={`campus-map-${c.id}`} className="block group">
            <div className="aspect-square overflow-hidden border border-[#580505]/15 bg-white">
              <img src={c.map} alt={t("campuses.map_alt", { name: c.name })} className="w-full h-full object-contain group-hover:scale-[1.02] transition-transform duration-300" />
            </div>
            <div className="text-xs text-[#580505] mt-1.5 inline-flex items-center gap-1">{t("campuses.map_zoom")} <ArrowRight size={11} /></div>
          </a>
        ) : (
      ...
      <div className="px-6 pb-6 flex flex-wrap gap-3">
        <Link to="/contact" data-testid={`campus-register-${c.id}`} className="btn-primary text-xs">{t("campuses.register_here")} <ArrowRight size={13} /></Link>
        <Link to="/contact" data-testid={`campus-contact-${c.id}`} className="btn-ghost text-xs">{t("campuses.contact_here")}</Link>
      </div>
    </article>
  );
}
  ...
  return (
    <article id={o.id} data-testid="campus-card-en-ligne" className="mpk-card flex flex-col bg-[#2F0808] text-white">
      <div className="px-6 py-5 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[11px] tracking-[0.2em] uppercase text-[#C4D2ED]">{t("campuses.online_subtitle")}</div>
          </div>
        </div>
      </div>
      <div className="p-6 space-y-4 flex-1">
        <p className="text-sm text-white/75 font-light leading-relaxed">{t("campuses.online_desc")}</p>
        <div>
          <div className="text-[10px] tracking-[0.2em] uppercase text-[#C4D2ED] font-medium mb-2">{t("campuses.online_slots_label")}</div>
      ...
      <div className="px-6 pb-6">
        <Link to="/cours-en-ligne" data-testid="campus-online-cta" className="inline-flex items-center gap-2 bg-white text-[#580505] px-4 py-2.5 text-xs font-medium hover:bg-[#C4D2ED]/90 transition">{t("campuses.online_cta")} <ArrowRight size={13} /></Link>
      </div>
    </article>
  );
}
...
export default function Campuses() {
  ...
  return (
    <>
      <PageHero eyebrow={t("campuses.hero_eyebrow")} title={t("campuses.hero_title")} kicker={t("campuses.hero_kicker")} />
